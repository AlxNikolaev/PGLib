...
namespace
{
	...
	/**
	 * Reference Poisson sampler: same RNG draws as FGeometryUtils::PoissonDiskSampling, but rejection scans every
	 * accepted point instead of a bucket ring, so a ring that is too narrow diverges from this sequence.
	 */
...
bool FClipVertexOnBoundaryTest::RunTest(const FString& Parameters)
{
	...
	// Kept side X ≤ 50: (0,0) inside, (100,0) outside, (50,100) exactly on the boundary.
...
bool FClipPlaneCoincidentVertexTest::RunTest(const FString& Parameters)
{
	...
	// Emitting each corner once leaves a two-vertex sliver and a failed clip, so the caller drops a zero-area
	// cell instead of accepting a "triangle" made of a duplicated point.
	{
...
bool FCentroidLShapeTest::RunTest(const FString& Parameters)
{
	...
	// The vertex average is (100,100); the area centroid is pulled toward the L's mass.
...
{
	// A 1000x1000 square keeps the result well clear of the default Epsilon of 10.
	TArray<FVector2D> Square = { FVector2D(0, 0), FVector2D(1000, 0), FVector2D(1000, 1000), FVector2D(0, 1000) };
	FVector2D		  Center;
	float			  Radius;
...
bool FPoissonMatchesLinearRejectionTest::RunTest(const FString& Parameters)
{
	...
	// The last case exercises the coarsened grid: 142x142 buckets against a 3840 budget, so the sampler coarsens
	// twice and narrows its neighbour ring to 1.
...
{
	// The thin axis fixes the cell size at 0.0025 while the float spacing at x = 2e6 is 0.125.
	// Height stays at 100 because a longer axis would reserve tens of megabytes of seed cells to prove the same thing.
